...
const Section6BuildSec5 = () => {
  ...
  useEffect(() => {
    ...
    return<Fragment>
       <div className="section-6-build-main-div"style={{backgroundColor: color[0]?.background_colour1 }}>
        <h1 style={{color: color[0]?.font_colour1}}>{items[0]?.shop_blockhead1}</h1>
        <p style={{color: color[0]?.font_colour2}}>{items[0]?.shop_block1}</p>
        <button style={{color: color[0]?.font_colour3, border: `2px solid ${color[0]?.background_colour2}`}}>Learn more</button>
       </div>
    </Fragment>
}

export default Section6BuildSec5