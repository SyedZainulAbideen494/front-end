...
import Template2userlist from "./template1list";
import { useParams } from "react-router-dom";

function Template2userapp() {
  ...
  const fetchprodshandler = useCallback(async () => {
    setloading(true);
    const response = await fetch("http://localhost:8080/user/template2", {
      headers: {
  ...
  return (
    <Fragment>
      <section>
        {!loading && <Template2userlist shops={items} />}
        {loading && <p>Loading..</p>}
      </section>
    </Fragment>
  );
...
export default Template2userapp;
