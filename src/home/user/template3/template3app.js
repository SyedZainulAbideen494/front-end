...
import Template3userlist from "./template3list";
import { useParams } from "react-router-dom";

function Template3userapp() {
  ...
  const fetchprodshandler = useCallback(async () => {
    setloading(true);
    const response = await fetch("http://localhost:8080/user/template3", {
      headers: {
  ...
  return (
    <Fragment>
      <section>
        {!loading && <Template3userlist shops={items} />}
        {loading && <p>Loading..</p>}
      </section>
    </Fragment>
  );
...
export default Template3userapp;
