...
import Template7userlist from "./template5list";
import { useParams } from "react-router-dom";

function Template7userapp() {
  ...
  const fetchprodshandler = useCallback(async () => {
    setloading(true);
    const response = await fetch("http://localhost:8080/user/template7", {
      headers: {
  ...
  return (
    <Fragment>
      <section>
        {!loading && <Template7userlist shops={items} />}
        {loading && <p>Loading..</p>}
      </section>
    </Fragment>
  );
...
export default Template7userapp;
