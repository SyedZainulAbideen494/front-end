...
import Template8userlist from "./template8list";
import { useParams } from "react-router-dom";

function Template8userapp() {
  ...
  const fetchprodshandler = useCallback(async () => {
    setloading(true);
    const response = await fetch("http://localhost:8080/user/template8", {
      headers: {
  ...
  return (
    <Fragment>
      <section>
        {!loading && <Template8userlist shops={items} />}
        {loading && <p>Loading..</p>}
      </section>
    </Fragment>
  );
...
export default Template8userapp;
