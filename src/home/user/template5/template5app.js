...
import Template5userlist from "./template5list";
import { useParams } from "react-router-dom";

function Template5userapp() {
  ...
  const fetchprodshandler = useCallback(async () => {
    setloading(true);
    const response = await fetch("http://localhost:8080/user/template5", {
      headers: {
  ...
  return (
    <Fragment>
      <section>
        {!loading && <Template5userlist shops={items} />}
        {loading && <p>Loading..</p>}
      </section>
    </Fragment>
  );
...
export default Template5userapp;
