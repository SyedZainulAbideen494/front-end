...
const shopId = params.shop_id
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.post('https://apifordropment.online/api/views/overview/shop', { shopId });
  ...
  const getTimeRangeOrders = (timeRange, ordersData) => {
    ...
    return ordersData.filter(order => new Date(order.visit_datetime) > filterDate);
  };
  ...
  const generateChartData = (timeRange, ordersData) => {
    ...
    filteredOrders.forEach((order) => {
      const orderDateTime = new Date(order.visit_datetime);
    ...
    <div className="main-content">
      <main className="main">
        <div className="container">
          <h2 className="section-title">Visits Overview</h2>
